...
# 查询在10和50之间的且状态为起售的菜品名称、价格、及其分类名称、原料
select dish.name, dish.price, category.name, dish.description
from dish
         left join category on dish.category_id = category.id
where dish.price between 10 and 50
  and dish.status = 1;

# 查询每个分类表下的最贵的菜品，展示出分类的名称，最贵的菜品价格

select dish.name, category.name, max(dish.price) as max_price
from category,
     dish
where dish.category_id = category.id
